...
source "$script_dir/a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_report_timing_core.tcl"


if [ info exists ddr_db ] {
   unset ddr_db
}
a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_corename}"
   } else {
      set fbasename "${::GLOBAL_a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_corename}_${inst_id}"
   }
   ...
   a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_perform_read_capture_analysis $opcname $inst pins var summary
   
   #######################################
   # Write Analysis

   a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis 
   
   a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # PHY Analyses
   
   a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance   

   set summary [lsort -command a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_sort_proc $summary]
   ...
   puts $f "Core: ${::GLOBAL_a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_corename} - Instance: $inst"
   puts $f "Path, Setup Margin, Hold Margin"

   
   post_message -type info "Core: ${::GLOBAL_a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ a10s_ghrd_altera_emif_arch_nf_181_ccplvsq_round_3dp $hold]
      }
